...
class VectorStore:
    ...
    def query(
        ...
        Correct conversion: similarity = 1.0 - (distance / 2.0)  ✓
        We also log raw distances at DEBUG level so you can tune the threshold.
        """
        ...
        for doc_id, document, metadata, distance in zip(
            ...
            # distance ∈ [0,2] for cosine space in ChromaDB
            similarity = max(0.0, 1.0 - (distance / 2.0))

            # ---------------------------------------------------------------- #
            # DEBUG: log every raw distance so you can spot threshold problems #
            # Run with LOG_LEVEL=DEBUG to see these lines.                     #
            # ---------------------------------------------------------------- #
            logger.debug(
                "chunk_id=%-40s  distance=%.4f  similarity=%.4f  heading=%s",
                doc_id,
                distance,
                similarity,
                (metadata or {}).get("section_heading", "?"),
            )
